...
var app = (0,vue__WEBPACK_IMPORTED_MODULE_0__.createApp)({
  ...
  methods: {
    initializePusher: function initializePusher() {
      var _this = this;
      var pusher = new Pusher("7772a4eda5202353109d", {
        cluster: "ap1",
        encrypted: true
      });
      var channel = pusher.subscribe('meeting.' + meetingId);
      channel.bind('UserJoinedMeeting', function (data) {
        _this.messages.push(data.message);
        console.log("".concat(data.user.name, " has joined the meeting."));
      });
      channel.bind('UserLeftMeeting', function (data) {
        _this.messages.push(data.message);
        console.log("".concat(data.user.name, " has left the meeting."));
      });
      channel.bind('MeetingEnded', function (data) {
        alert(data.message);
