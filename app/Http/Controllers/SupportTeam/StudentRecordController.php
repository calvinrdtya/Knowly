...
class StudentRecordController extends Controller
{
    ...
    public function schedule()
    {
        ...
        if (!$myClassId) {
            return redirect()->route('dashboard')->with('error', 'Kamu belum memiliki kelas yang terkait.');
        }

        $subjects = Subject::where('my_class_id', $myClassId)->get();

        if ($subjects->isEmpty()) {
            return redirect()->route('dashboard')->with('error', 'Tidak ada mata pelajaran yang tersedia untuk kelas ini.');
        }

        $data = [
            'subjects' => $subjects,
            'class' => MyClass::find($myClassId),
        ];

        return view('student.schedule.index', $data);
    }
